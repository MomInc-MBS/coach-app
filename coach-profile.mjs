...
gate.innerHTML='<h1>Connecting…</h1><p></p><a href="/pose.html?reconnect=1" style="color:#b8e9cf">Reload Coach</a><a id="gateSignIn" href="#" style="display:none;color:#b8e9cf;margin-left:16px">Sign in</a>';document.body.append(gate);gate.showModal();
let applied='',restored=false,localPlan=null;
// #15: a bounced ?optional= route (launch.mjs) hands its one-line reason here so it lands on the
// gate too if the gate ends up the thing blocking the visitor. Read once, then forgotten, so a later
// unrelated sign-out doesn't repeat a stale route name.
let optionalReason='';
export function setOptionalGateReason(text){optionalReason=text;}
function withOptionalReason(text){if(!optionalReason)return text;const combined=text+' '+optionalReason;optionalReason='';return combined;}
...
export function clearCoachAccount(){window.myr5AuthenticatedAccount=null;window.coachEntitlements=null;window.myr5VerifiedOptionalAccess=false;document.documentElement.dataset.publicState='locked';window.coachProgress=null;window.dispatchEvent(new Event('myr5:account-cleared'));window.coachPlan=localPlan;restored=false;applied='';if(localPlan){gate.close();gate.hidden=true;gate.style.display='none';window.dispatchEvent(new Event('myr5:coach-plan'));return;}
 // No saved plan on this device: Coach cannot render without one, so the gate must keep blocking —
 // but it now says why and always offers a working Sign in (reconnect) alongside Get started (#10/#11).
 gate.hidden=false;gate.style.display='grid';if(!gate.open)gate.showModal();gate.querySelector('h1').textContent='Set up Coach';gate.querySelector('p').textContent=withOptionalReason('Sign in to reconnect your saved coach, or get started fresh — Coach needs a plan before it can open.');gate.querySelector('a').textContent='Get started';gate.querySelector('a').href=beginSetup();const signIn=gate.querySelector('#gateSignIn');signIn.href=signInPath('/onboarding.html');signIn.style.display='';}
...
export function applyCoachAccount(account){
 ...
 // #11: signed in but setup isn't finished — say why it blocks (not just what to do next).
 if(!o){gate.hidden=false;gate.style.display='grid';if(!gate.open)gate.showModal();gate.querySelector('#gateSignIn').style.display='none';gate.querySelector('h1').textContent='Set up Coach';gate.querySelector('p').textContent=withOptionalReason('Coach needs your setup finished before it can open — choose games or quick setup.');gate.querySelector('a').href='/onboarding.html';gate.querySelector('a').textContent='Continue coach setup';gate.querySelector('a').href=beginSetup();if(setupAllowed())showIncomingCoach();return;}
