...
gate.innerHTML='<h1>Connecting…</h1><p></p><a href="/onboarding.html" style="color:#b8e9cf">Continue coach setup</a>';document.body.append(gate);gate.showModal();
...
export function clearCoachAccount(){beginSetup();window.coachPlan=null;restored=false;applied='';gate.hidden=false;gate.style.display='grid';if(!gate.open)gate.showModal();gate.querySelector('h1').textContent='Set up Coach';gate.querySelector('p').textContent='Choose games or quick setup.';gate.querySelector('a').href='/onboarding.html';gate.querySelector('a').textContent='Choose games or paperwork';showIncomingCoach();}
export function applyCoachAccount(account){
 const o=account.onboarding;
 if(!o){beginSetup();gate.hidden=false;gate.style.display='grid';if(!gate.open)gate.showModal();gate.querySelector('h1').textContent='Set up Coach';gate.querySelector('p').textContent='Choose games or quick setup.';gate.querySelector('a').href='/onboarding.html';gate.querySelector('a').textContent='Choose games or paperwork';showIncomingCoach();return;}
