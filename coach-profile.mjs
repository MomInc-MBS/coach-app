...
export function clearCoachAccount(){beginSetup();window.coachPlan=null;restored=false;applied='';gate.hidden=false;gate.style.display='grid';if(!gate.open)gate.showModal();gate.querySelector('h1').textContent='Your coach is waiting.';gate.querySelector('p').textContent='Got Coach from a friend? Play the character games or fill out the office form to build your own coach.';gate.querySelector('a').href='/onboarding.html';gate.querySelector('a').textContent='Choose games or paperwork';showIncomingCoach();}
export function applyCoachAccount(account){
 const o=account.onboarding;
 if(!o){beginSetup();gate.hidden=false;gate.style.display='grid';if(!gate.open)gate.showModal();gate.querySelector('h1').textContent='Your coach needs its final setup.';gate.querySelector('p').textContent='Choose the character games or our gloriously dull office form. Both collect your answers and build your coach.';gate.querySelector('a').href='/onboarding.html';gate.querySelector('a').textContent='Choose games or paperwork';showIncomingCoach();return;}
