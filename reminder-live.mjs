...
export function mountLiveReminders({refresh,read,getAccount,deviceReady=()=>true}){
 ...
 function render(){
  ...
  status.textContent=!navigator.onLine?'Offline · reminders resume when connected':!account?'Sign in to receive reminders':!fresh?'Checking the reminder connection…':!account.push.configured?'Notification service is unavailable':!account.push.schedulerActive?'Reminder sender is reconnecting':!on?'Turn on notifications for this device':!deviceReady()?'Reconnecting this device…':!rows.some(r=>r.enabled)?'Connected · save or enable a reminder':next?`Live · ${next.r.kind} · ${reminderCountdown(next.next,now)}`:'Connected · choose a time outside quiet hours';
 }
