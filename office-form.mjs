import {OFFICE_STYLES,officeLine} from './office-domain.mjs?v=office-short-v1';
const node=(tag,text)=>{const e=document.createElement(tag);if(text)e.textContent=text;return e;};
export function mountOfficeAppearance(form,data,{optional=false}={}){
 ...
 const appearance=node('fieldset');appearance.append(node('legend','Coach appearance'));
 ...
 const banterLabel=node('label');banterLabel.className='office-check';const banter=node('input');banter.type='checkbox';banter.name='officeBanter';banter.checked=data.officeBanter===true;banterLabel.append(banter,document.createTextNode('Let my coach roast my choice of paperwork.'));if(!optional)appearance.append(banterLabel,node('p','Uncheck for quiet paperwork. “Quiet” guidance and requests for no teasing also silence the jokes.'));
 return {appearance,banterLabel,collect(){for(const k of Object.keys(recipe.styles))recipe.styles[k]=Number(form.elements.namedItem('appearance.'+k).value);recipe.eye=eyes.value;recipe.coach=data.profile.coach||'supportive';data.customizationConfirmed=true;data.officeBanter=banter.checked;},paint(missing,total){mascot.dataset.eye=recipe.eye;const next=officeLine(data,missing===0?4:Math.min(3,Math.floor((1-missing/total)*4)));if(line.textContent!==next)line.textContent=next;}};
}
