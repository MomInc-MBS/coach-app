...
$('copyInstallLink').onclick=async()=>{try{await navigator.clipboard.writeText($('installLink').value);set('installShareStatus','Link copied.');}catch{$('installLink').focus();$('installLink').select();set('installShareStatus','Copy the selected link.');}};
try{localStorage.setItem('myr5-voice-style','robot');}catch{}
$('downloadVoice').onclick=async()=>{const b=$('downloadVoice');b.disabled=true;try{const manifest=await(await fetch(VOICE_MANIFEST)).json(),urls=Object.values(manifest.phrases),cache=await caches.open(VOICE_CACHE);let i=0;await cache.add(VOICE_MANIFEST);for(const url of urls){if(!await cache.match(url)){const response=await fetch(url);if(!response.ok)throw Error('Interrupted. Tap to resume.');await cache.put(url,response);}set('downloadStatus',`${++i} / ${urls.length}`);}set('downloadStatus','Voice ready offline.');}catch(e){set('downloadStatus',e.message);}finally{b.disabled=false;}};
