...
window.coachAccount={async start(mode,goal){const ready=account||await refresh();if(!ready)throw Error('Sign in from Progress before starting a saved workout.');if(!ready.onboarding)throw Error('Complete your coach setup first.');await flushSets();return api('/api/workouts/start','POST',{mode,goal});},async complete(data){if(!account)return;const queue=pending(account.user.id);if(!queue.some(x=>x.id===data.id)){queue.push(data);localStorage.setItem(pendingKey(account.user.id),JSON.stringify(queue));}await flushSets();}};
...
$('mealForm').onsubmit=async e=>{e.preventDefault();const form=e.target,submit=form.querySelector('[type=submit]');submit.disabled=true;$('foodCamera').disabled=true;const d={...Object.fromEntries(new FormData(form)),...mealNutrition.data()};try{d.id=crypto.randomUUID();d.eatenAt=new Date(d.eatenAt).toISOString();await api('/api/meals','POST',d);set('mealStatus','Meal saved to your account.');mealNutrition.reset();form.elements.eatenAt.value=localDate();$('foodStatus').textContent='Meal saved. Take another photo when you are ready.';await meals();}catch(err){set('mealStatus',err.message);}finally{submit.disabled=false;$('foodCamera').disabled=false;}};
...
async function syncDeviceSwitch(){
 ...
 try{
  ...
  set('pushStatus',!supported?'Install Coach on your home screen to enable notifications.':!account?'Sign in to enable reminders.':Notification.permission==='denied'?'Allow notifications for Coach in your browser or phone settings.':!account.push.configured?'Notification service is reconnecting.':!account.push.schedulerActive?'The background sender is reconnecting. Saved reminders are kept.':on?'This device is connected. Reminders run even when Coach is closed.':'The background sender is running. Turn this device on to receive reminders.');
  liveReminders.render();
 }catch{deviceBinding.forget();$('testPush').disabled=true;set('pushStatus','This device could not reconnect. We will retry automatically.');liveReminders.render();}
}
