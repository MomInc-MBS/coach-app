...
window.addEventListener('online',refresh);document.addEventListener('visibilitychange',()=>{if(!document.hidden)refresh();});
 await applyLocalCoach();void refresh();const panel=new URLSearchParams(location.search).get('panel');if(['history','meals','reminders','account','install'].includes(panel))document.querySelector(`[data-panel=${panel}]`).click();
