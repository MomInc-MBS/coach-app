...
export class SetFlow {
 ...
 tap(now,withHand=false){if(this.phase!=='rest'||now-this.lastTap<180)return null;this.lastTap=now;this.hits++;const assisted=withHand&&this.hits%3===0,damage=weaponDamage(this.combat,this.weapon,now);this.damage+=damage;return {hits:this.hits,damage,totalDamage:this.damage,blocked:damage===0,assisted,charge:withHand?this.hits%3:0};}
 special(weapon,{now=Date.now(),progress,catalog}={}){const result=this.abilities.activate(weapon,{now,progress,catalog,inRest:this.phase==='rest'});if(!result.ok)return result;const damage=weaponDamage(this.combat,weapon,now);this.damage+=damage;return {...result,special:true,assisted:false,damage,totalDamage:this.damage,blocked:damage===0,hits:this.hits};}
