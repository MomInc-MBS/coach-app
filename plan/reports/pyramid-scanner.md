...
Nutrition follows the selected food reference and current portion (initially 100 g), corrected name and manual label edits in the existing meal form. A scan with no clear match leaves the nutrition fields blank. Camera-only workout behavior is unchanged.
...
- The background author's build measured 266,543,141 bytes of deployment file payload, not the tar stream including archive headers/padding. Core offline install was 3,278,714 bytes at that point. Rebuild and check the final packaged archive after integration. The room adds no binary or separately inventoried asset; its CSS/DOM is 2,881 bytes of source inside the existing lazy scanner module.
