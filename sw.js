...
// Every creature model ships with the app (no loading screens). scripts/build.mjs fills this line with
// content-hashed URLs; a code-only release reuses models already cached under the same hash.
const MODEL_CACHE='myr5-models-dev',MODELS=[];
...
FILES.push('/public-entry.mjs','/pocket-hardware.css','/reminder-settings.mjs','/reminder-controls.mjs','/meal-scanner.mjs','/food-worker.mjs','/icons/coach-install-qr.png','/pod/hardware.css','/pod/hardware.mjs','/hardware-launch.css','/pod/whiteboard.css','/pod/fonts/hand-0.woff2','/pod/fonts/hand-2.woff2','/pod/fonts/hand-6.woff2');
...
FILES.push('/coach-hub.mjs','/coach-hub.css','/reminder-plan.mjs','/pod/mom-inc-mark-96.webp','/voice/manifest.json');
FILES.push('/pod/home-character.mjs','/pod/home-character.css','/pod/gala-performer.js');
const CORE=['/pose.html','/manifest.webmanifest','/app-runtime.mjs','/launch-bootstrap.mjs','/launch-runtime.mjs','/workout-tracks.js','/pod/gala-weapons.js','/pod/gala-avatar.js','/pod/gala-performer.js','/pod/dj-identity.js','/icons/myr5-alien-192.png','/pod/mom-inc-mark-96.webp'];
const OPTIONAL_INITIAL=['/handborne/','/creature/','/war-room','/warroom','/character-editor/','/editor/','/pocket-hardware.css','/hardware-launch.css','/pod/hardware.css','/pod/hardware.mjs','/pod/whiteboard.css'];
const INITIAL_FILES=FILES.filter(file=>!OPTIONAL_INITIAL.some(prefix=>file===prefix||file.startsWith(prefix)));
// Tier 1 (atomic): what the pod needs to open. Tier 2 (best effort): everything else, so one slow file never fails the install.
self.addEventListener('install',event=>event.waitUntil(caches.open(SHELL).then(async cache=>{await cache.addAll(INITIAL_FILES.filter(f=>CORE.includes(f)));await Promise.allSettled(INITIAL_FILES.filter(f=>!CORE.includes(f)).map(f=>cache.add(f)));
 const models=await caches.open(MODEL_CACHE);await Promise.allSettled(MODELS.map(async url=>{if(await models.match(url))return;const hit=await caches.match(url);await(hit?models.put(url,hit):models.add(url));}));})));
self.addEventListener('activate',event=>event.waitUntil((async()=>{for(const key of await caches.keys())if((key.startsWith('myr5-shell-')&&key!==SHELL)||(key.startsWith('myr5-voice-')&&key!==VOICE)||(key.startsWith('myr5-models-')&&key!==MODEL_CACHE))await caches.delete(key);await self.clients.claim();})()));
...
self.addEventListener('fetch',event=>{const request=event.request,url=new URL(request.url);if(request.method!=='GET'||url.origin!==self.location.origin||url.pathname.startsWith('/api/')||url.pathname.includes('with-chatgpt')||url.pathname==='/callback'||url.pathname.startsWith('/signin')||url.pathname==='/source.json')return;
 ...
 // Models are cached as <file>?v=<hash> while the app asks for <file>; they stay out of the shell cache.
 if(url.pathname.endsWith('.glb')&&MODELS.length){event.respondWith((async()=>(await caches.match(request,{ignoreSearch:true}))||fetch(request))());return;}
