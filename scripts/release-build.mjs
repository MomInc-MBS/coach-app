...
export async function releaseBuildId(root='.'){
 ...
 for(const entry of (await readdir(root,{withFileTypes:true})).sort((a,b)=>a.name.localeCompare(b.name)))if(entry.isFile()&&entry.name!=='release-build.mjs'&&/\.(html|css|mjs|js|webmanifest|json)$/.test(entry.name))await add(entry.name);
