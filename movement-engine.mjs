...
export const MOVEMENTS = {
  ...
  warrior: { name: 'Warrior II', kind: 'hold', hint: 'Show your arms, hips and knees. Either direction works; feet are optional.' },
  horse: { name: 'Horse stance', kind: 'hold', hint: 'Show your hips and knees in your comfortable wide stance. Feet are optional.' },
...
const SIDES = { left: { s:11,e:13,w:15,h:23,k:25,a:27 }, right: { s:12,e:14,w:16,h:24,k:26,a:28 } };
...
const JOINTS={11:'left shoulder',12:'right shoulder',13:'left elbow',14:'right elbow',15:'left hand',16:'right hand',23:'left hip',24:'right hip',25:'left knee',26:'right knee',27:'left foot',28:'right foot'};
...
export function features(raw,aspect=1,world=null){
  ...
  for(const [name,id] of Object.entries(SIDES)){
    ...
    sides[name]={...q,id,torso,
      ...
      leg:visible([id.s,id.h,id.k,id.a])&&torso>.035,
      arm:visible([id.s,id.e,id.w,id.h])&&torso>.035,
      quality:Math.min(...[id.s,id.h,id.k,id.a].map(i=>p[i].visibility)),
      armQuality:Math.min(...[id.s,id.e,id.w,id.h].map(i=>p[i].visibility)),
      knee:jointAngle(q.h,q.k,q.a),elbow:jointAngle(q.s,q.e,q.w),
      hipHeight:(q.a.y-q.h.y)/Math.max(torso,.035),
      horizontal:Math.abs(q.s.x-q.h.x)/Math.max(torso,.035)
    };
  }
  const torso=(sides.left.torso+sides.right.torso)/2;
  const hip=midpoint(p[23],p[24]),ankle=midpoint(p[27],p[28]),shoulder=midpoint(p[11],p[12]);
  return {p,world,visible,issues,sides,torso,hip,ankle,shoulder,
    upright:(hip.y-shoulder.y)/Math.max(torso,.035)>.65,
    knees:visible([11,12,23,24,25,26])&&torso>.035,
    fullLegs:visible([11,12,23,24,25,26,27,28])&&torso>.035};
}
...
export class MovementSession {
  ...
  reset(mode=this.mode,options={}){
    ...
    this.candidate=null;this.missAt=null;this.previousMatch=false;this.hands={};this.footArmed={left:false,right:false};this.lastStep=-Infinity;
  ...
  lose(t,message='Keep the needed joints in view.'){
    this.tracking=false;this.message=message;this.measurement='Tracking paused';this.phase='ready';this.phaseSince=null;this.filtered={};this.calibration=[];this.setupProgress=0;this.progress=0;this.footArmed={left:false,right:false};this.hands={};this.speed=0;
  ...
  selectSide(f,kind,t){
    ...
    if(!candidates.length){
      const required=s=>kind==='core'?[s.id.s,s.id.h]:kind==='leg'?[s.id.s,s.id.h,s.id.k,s.id.a]:[s.id.s,s.id.e,s.id.w,s.id.h];
    ...
    const key=kind==='core'?'coreQuality':kind==='leg'?'quality':'armQuality';
  ...
  calibrate(sample,ready,t){
    ...
    const key=sample.hipY!==undefined?'hipY':sample.angle!==undefined?'angle':'ankle';
  ...
  repetition(down,up,t,minDuration){
    if(this.phase==='ready'){
      if(up){this.phase='top';this.phaseSince=null;}return;
    }
    if(this.phase==='top'){
      if(down){if(this.phaseSince===null)this.phaseSince=t;if(t-this.phaseSince>=.10){this.phase='bottom';this.phaseSince=null;this.downAt=t;}}else this.phaseSince=null;
  ...
  repUpdate(f,t,dt){
    ...
    this.tracking=true;
    const kneeVisible=f.visible([s.id.h,s.id.k,s.id.a]);
    const a=this.smooth('angle',squat?(kneeVisible?s.knee:180):s.elbow,dt),hipY=this.smooth('hipY',s.h.y,dt);
    if(!this.base){
      const ready=squat?(s.h.y-s.s.y)/s.torso>.65:a>145&&s.horizontal>=.5;
      const calibrated=this.calibrate(squat?{hipY,torso:s.torso,angle:a,kneeVisible:Number(kneeVisible)}:{angle:a},ready,t);
    ...
    const useKnee=squat&&kneeVisible&&this.base.kneeVisible;
    const bend=squat?(useKnee?this.base.angle-a:0):this.base.angle-a,drop=squat?(hipY-this.base.hipY)/this.base.torso:0;
    // Hip travel relative to the starting torso supports cropped frontal views.
    // Optional knee readings must never veto a hip cycle when feet are cropped.
    ...
    this.measurement=squat?`${this.side} hip · travel ${Math.max(0,drop).toFixed(2)}${useKnee?' · knee available':' · feet optional'}`:`${this.side} arm · bend ${Math.round(bend)}°`;
  }
  holdUpdate(f,t,dt){
  ...
  boxingUpdate(f,t,dt){
    const available=Object.entries(f.sides).filter(([,s])=>s.arm);
    ...
    this.message=this.speed>.7?'Movement detected. Keep your own rhythm.':'Timer running. Move your hands when ready.';
    this.measurement='Hand pace is relative to torso length. No punch-accuracy score.';this.progress=clamp(this.speed/5,0,1);
  }
  jogUpdate(f,t,dt){
    if(!f.knees){this.lose(t,'Steps paused: '+(f.issues([11,12,23,24,25,26]).slice(0,2).join('; ')||'move a little closer')+'. Feet are optional.');return;}
    this.tracking=true;const l=f.sides.left,r=f.sides.right;
    for(const [name,a,b] of [['left',l,r],['right',r,l]]){
      const lift=this.smooth(name+'Lift',(b.k.y-a.k.y)/f.torso,dt);
      if(lift<.05)this.footArmed[name]=true;
      if(this.footArmed[name]&&lift>.13&&t-this.lastStep>.17){this.count++;this.eventTimes.push(t);this.lastStep=t;this.footArmed[name]=false;}
    }
    this.message='Counting alternating knee lifts.';this.measurement='Estimated steps from knees; feet are optional.';this.progress=clamp(Math.abs(l.k.y-r.k.y)/f.torso/.3,0,1);
  }
  jumpUpdate(f,t,dt){
  ...
  update(raw,timestamp,aspect=1,world=null){
    ...
    this.jointReadings=f?Object.entries(f.sides).map(([side,s])=>`${side}: `+[s.id.s,s.id.h,s.id.k].map(i=>`${JOINTS[i].split(' ')[1]} ${Math.round(f.p[i].visibility*100)}%${f.visible([i])?'':' ×'}`).join(', ')).join(' | ')+' · Feet optional in every mode':'No body landmarks';
    ...
    else if(MOVEMENTS[this.mode].kind==='hold')this.holdUpdate(f,t,dt);
    else if(this.mode==='boxing')this.boxingUpdate(f,t,dt);
    else if(this.mode==='jogging')this.jogUpdate(f,t,dt);
