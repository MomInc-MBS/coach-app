...
export async function unpackVerifiedBundle(value) {
  ...
  if (bytes.byteLength < 6 || bytes.byteLength>64*1024*1024) throw new Error('verified bundle size exceeds policy');
  ...
  for (const [name,entry] of Object.entries(index)) {
    if (!/^[A-Za-z0-9][A-Za-z0-9._/-]*$/.test(name) || name.includes('..') || name.includes('//') || name.endsWith('/') || name.length>300 || !entry || !Number.isSafeInteger(entry.offset) || entry.offset!==expectedOffset || !Number.isSafeInteger(entry.bytes) || entry.bytes<1 || !/^[a-f0-9]{64}$/.test(entry.sha256||'')) throw new Error(`invalid bundled entry: ${name}`);
