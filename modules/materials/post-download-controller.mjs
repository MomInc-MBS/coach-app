...
export async function downloadPostDownloadSection({id,account=globalThis.myr5AuthenticatedAccount,authorize=()=>false,fetchImpl=globalThis.fetch,store=indexedDbChunkStore(),policy=DEFAULT_LOCAL_RESOURCE_POLICY,signal,onProgress}={}){
 ...
 const resolved=await resolvePostDownloadSection(id,{fetchImpl,policy});
 const allowed=()=>getOwner(globalThis.myr5AuthenticatedAccount??account)===owner&&authorize(id,globalThis.myr5AuthenticatedAccount??account);
