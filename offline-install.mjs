// Installation is ready only after the worker has saved the complete artwork.
export async function prepareOfflineApp(serviceWorker=globalThis.navigator?.serviceWorker){
 ...
 await new Promise((resolve,reject)=>{
  function check(){
   ...
   else if(worker.state==='redundant'){worker.removeEventListener('statechange',check);reject(Error('Coach artwork could not be saved. Check your connection and free space, then retry.'));}
  }
