export function mountCameraWorkout({video,counter,onStop}){
 ...
 const stop=document.createElement('button');stop.type='button';stop.className='camera-workout-counter';stop.setAttribute('aria-label','Stop workout');stop.title='Tap the counter to stop';stage.append(stop);document.body.append(stage);
