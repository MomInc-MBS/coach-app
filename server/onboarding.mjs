...
export async function saveOnboarding(database,user,input,now=Date.now(),{dataEpoch=1}={}){
 ...
 const writes=await epochFencedBatch(database,{ownerId:user,expectedDataEpoch:dataEpoch,now,statements:[
 ...
 const onboarding=await readOnboarding(database,user,now);
 if((await inspectAccountDataEpoch(database,user)).currentDataEpoch!==dataEpoch)throw Object.assign(Error('Account data changed.'),{status:409,code:'target_epoch_mismatch'});
