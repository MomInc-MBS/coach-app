...
async function scores(database,user){return (await database.prepare('SELECT mode,COUNT(*) AS sets,MAX(value) AS best FROM workouts WHERE user_id=? AND completed_at IS NOT NULL GROUP BY mode').bind(user).all()).results;}
...
export async function scoreboardApi(database,user,path,method,input={},now=Date.now()){
 ...
 if(path==='/api/scoreboard/invite'&&method==='POST'){
  ...
  const saved=await database.prepare('INSERT INTO scoreboard_invites(owner_id,token_hash,created_at,expires_at) VALUES(?,?,?,?) ON CONFLICT(owner_id) DO UPDATE SET token_hash=excluded.token_hash,created_at=excluded.created_at,expires_at=excluded.expires_at WHERE scoreboard_invites.created_at<=? RETURNING owner_id').bind(user,await hash(code),now,now+7*86400000,now-60000).first();
 ...
 if(path==='/api/scoreboard/invite'&&method==='DELETE'){
  await database.prepare('DELETE FROM scoreboard_invites WHERE owner_id=?').bind(user).run();return {revoked:true};
 }
 if(path==='/api/scoreboard/join'&&method==='POST'){
  ...
  const rate=await database.prepare("INSERT INTO system(key,value) VALUES(?,json_object('since',?,'count',1)) ON CONFLICT(key) DO UPDATE SET value=CASE WHEN json_extract(system.value,'$.since')<? THEN excluded.value ELSE json_set(system.value,'$.count',json_extract(system.value,'$.count')+1) END WHERE json_extract(system.value,'$.since')<? OR json_extract(system.value,'$.count')<20 RETURNING key").bind(`scoreboard-join:${user}`,now,now-3600000,now-3600000).first();
 ...
 if(path.startsWith('/api/scoreboard/friends/')&&method==='DELETE'){
  const id=cleanText(path.split('/').at(-1),50);const removed=await database.prepare('DELETE FROM scoreboard_links WHERE id=? AND (user_a=? OR user_b=?) RETURNING id').bind(id,user,user).first();
