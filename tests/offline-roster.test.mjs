...
test('D34: core is only what first-run pages name; bundled sources, deferred features and voice go to the package',async()=>{
 ...
 try{
  ...
  await writeFile(join(root,'app-runtime.mjs'),"import('./nutrition-data.mjs');img.src='/pod/mom-inc-mark.png';new URL('./creature/assets/phone.js',import.meta.url);");
  ...
  for(const file of ['nutrition-data.mjs','meditation.mjs','pod/mom-inc-mark.png','pod/fonts/hand-0.woff2','pod/worlds/wall.png','icons/used.png','icons/unused.png','voice/a.mp3'])await writeFile(join(root,file),'x'+file);
