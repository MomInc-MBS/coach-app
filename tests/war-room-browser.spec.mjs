...
test.beforeAll(async()=>{server=http.createServer(async(req,res)=>{const path=new URL(req.url,'http://local').pathname;
 ...
 // W1-1C #15: a locked deep link used to bounce silently to the pod. It must still land there (this
 // is not a security boundary, the server/account stays authoritative), but the URL now names which
 // route it was and why, so the page it lands on (launch.mjs) can show "Finish Coach setup to unlock
 // the War Room." instead of nothing.
 test('locked deep link lands on the pod naming the route it bounced from',async({page})=>{unlocked=false;await page.goto(base+'/war-room/');await page.waitForURL(url=>url.pathname==='/pose.html'&&url.searchParams.get('optional')==='/war-room');});
