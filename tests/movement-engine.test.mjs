...
test('portrait aspect is corrected before joint angles are calculated',()=>{const p=squat();const adjusted=p.map(v=>({...v,x:v.x/.75}));assert.ok(Math.abs(features(p,1).sides.left.knee-features(adjusted,.75).sides.left.knee)<1e-9);});
