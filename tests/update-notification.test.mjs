...
test('update notifications offer download, open a closed app, and preserve an open workout',async()=>{
 ...
 // A closed app opens recovery first, so an unresponsive install can still update.
 await click(shown[0].options.data);assert.equal(opened[0],'https://coach.test/repair-coach');
 clients=[{url:'https://coach.test/pose.html',focus:async()=>focused++,navigate:async()=>navigated++,postMessage:(message,ports)=>{messages.push(message);ports?.[0]?.postMessage({handled:true});ports?.[0]?.close();}}];
