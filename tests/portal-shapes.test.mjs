// Recognizer tests for the production shape module (unbundled, dynamic-imported by the app — see
// modules/portal/portal.mjs). Helpers and closed-shape fixtures are ported from
// plan/prototypes/portal-boards/tests/portal-shapes.test.mjs, whose SHAPES templates are identical
...
// One-way line from a to b (used for x/cross strokes and for the direction-aware single-stroke line).
...
test('exports the twelve shape ids, lines split into four directions',()=>{
 assert.deepEqual([...SHAPE_IDS].sort(),['cross','down','hdiamond','line-down','line-lr','line-rl','line-up','oval','rect','up','vdiamond','x'].sort());
});
...
test('recognizes the two-stroke X and center cross in either stroke order',()=>{
...
test('a single-stroke line is direction-aware: vertical -> line-down/line-up, horizontal -> line-lr/line-rl',()=>{
 assert.equal(recognizeShape([line([.5,.03],[.5,.97])]),'line-down','vertical, traced top to bottom');
 assert.equal(recognizeShape([line([.5,.97],[.5,.03])]),'line-up','vertical, traced bottom to top');
 assert.equal(recognizeShape([line([.03,.5],[.97,.5])]),'line-lr','horizontal, traced left to right');
 assert.equal(recognizeShape([line([.97,.5],[.03,.5])]),'line-rl','horizontal, traced right to left');
 // Direction reads the raw stroke's first vs last point, not the fitted template: a slanted, jittered,
 // off-centre trace must still resolve, in both directions.
 assert.equal(recognizeShape([line([.56,.05],[.53,.95],.03)]),'line-down','slanted, off-centre, top to bottom');
 assert.equal(recognizeShape([line([.53,.95],[.56,.05],.03)]),'line-up','slanted, off-centre, bottom to top');
 assert.equal(recognizeShape([line([.05,.53],[.95,.48],.03)]),'line-lr','slanted, off-centre, left to right');
 assert.equal(recognizeShape([line([.95,.48],[.05,.53],.03)]),'line-rl','slanted, off-centre, right to left');
});

test('sparse fast swipes are filled in before matching and keep their direction',()=>{
 assert.equal(recognizeShape([[[.5,.03],[.5,.4],[.5,.75],[.5,.97]]]),'line-down');
 assert.equal(recognizeShape([[[.5,.97],[.5,.75],[.5,.4],[.5,.03]]]),'line-up');
 assert.equal(recognizeShape([[[.03,.5],[.4,.5],[.75,.5],[.97,.5]]]),'line-lr');
 assert.equal(recognizeShape([[[.97,.5],[.75,.5],[.4,.5],[.03,.5]]]),'line-rl');
