...
test('pinned raw-host CORS ranges use signed chunk hashes when Content-Range is browser-hidden',async()=>{
 ...
 try{
  ...
  const run=async({assetUrl=url,policyPatch={},etag,loseOwner=false,finalUrl}={})=>{
   requests=[];return page.evaluate(async({manifest,publicJwk,policy,loseOwner,finalUrl})=>{
    ...
    const downloader=new ChunkDownloader({store,manifestPublicKey:publicJwk,expectedVersion:'1.0.0',ownership:()=>owner,policy,fetchImpl:async(url,options)=>{const response=await fetch(url,options);seen.push({type:response.type,url:response.url,range:response.headers.get('Content-Range'),length:response.headers.get('Content-Length'),redirect:options.redirect});if(loseOwner)owner=null;return finalUrl===undefined?response:{ok:response.ok,status:response.status,redirected:response.redirected,type:response.type,headers:response.headers,body:response.body,url:finalUrl};}});
    try{const result=await downloader.download(manifest,{retry:0}),asset=await downloader.readVerifiedAsset(manifest,'assets/arms.m5bundle');return {ok:true,received:result.receivedBytes,bytes:[...asset.bytes],used:store.usedBytes,seen};}catch(error){return {ok:false,error:error.message,used:store.usedBytes,seen};}
   },{manifest:await makeManifest(assetUrl,etag),publicJwk,policy:{...DEFAULT_LOCAL_RESOURCE_POLICY,trustedOrigins:['https://raw.githubusercontent.com'],allowedProtocols:['https:'],pathPrefix:prefix,...policyPatch},loseOwner,finalUrl});
  };
