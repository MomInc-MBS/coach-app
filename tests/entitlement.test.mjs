...
test('account entitlement is locked until trusted completion, then durable and replay-idempotent',async()=>{
 ...
 const replay=await request('/api/integrations/coach-army/completion',{method:'POST',secret:env.COACH_ARMY_COMPLETION_SECRET,data:{...event,completedAt:1700000000001}});assert.equal(replay.status,200);assert.deepEqual((await request('/api/account')).data.entitlements.coachArmy,granted);
