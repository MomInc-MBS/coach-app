...
test('scheduled notifications encrypt real push payloads and send only once without a browser',async()=>{
 ...
 globalThis.fetch=async(url,options)=>{if(url===sub.endpoint){sent++;assert.equal(options.redirect,'manual');const h=new Headers(options.headers);assert.match(h.get('authorization'),/^vapid /);assert.equal(h.get('content-encoding'),'aes128gcm');assert(options.body);return new Response(null,{status:201});}return originalFetch(url,options);};
