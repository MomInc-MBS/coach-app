...
test('signed installed skin survives the real rig shader handoff, offline reload, and account teardown',async()=>{
 ...
 try{
  ...
  offline=true;await page.reload();await ready();assert.equal(await page.locator('#tab-skin').isVisible(),true,'verified cached metadata permits offline reload');
  await page.evaluate(async()=>{window.disposals=0;for(const texture of myr5Companion.viewer.skinTextures)texture.addEventListener('dispose',()=>window.disposals++);window.textureCount=myr5Companion.viewer.skinTextures.size;await fetch('/__account/owner-b');window.dispatchEvent(new Event('focus'));});await page.waitForFunction(()=>window.myr5AuthenticatedAccount?.user?.id==='owner-b'&&myr5Companion.ready&&myr5Companion.viewer.skinTextures.size===0);assert.equal(await page.locator('#tab-skin').isHidden(),true);assert.equal(await page.evaluate(()=>disposals),await page.evaluate(()=>textureCount));assert.equal(await page.locator('#skinChoice option').count(),0);assert.equal(await page.locator('#tab-ship').isHidden(),true);
