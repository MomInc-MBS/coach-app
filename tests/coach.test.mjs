...
function queuedVoice(clips){const voice=new CoachVoice(()=>{});voice.available=true;voice.robot={unlock:()=>true,stop:()=>{},play:text=>new Promise(resolve=>clips.push({text,onend:()=>resolve(true)}))};return voice;}
...
test('legacy voice clips remain usable and every movement has a finite procedural preview',()=>{for(const key of Object.keys(INTRO))assert.ok(MOVEMENTS[key]);for(const mode of Object.keys(MOVEMENTS))for(const t of [0,.3,1,2]){const p=demoPose(mode,t);for(const [a,b] of DEMO_BONES){assert.ok(p[a].every(Number.isFinite),mode);assert.ok(p[b].every(Number.isFinite),mode);}}});
test('speech replaces stale pending counts and cancelling resolves every cue',async()=>{
 const utterances=[],voice=queuedVoice(utterances);const promises=[voice.say('30 seconds left.',{key:'time'}),voice.say('1',{key:'count'}),voice.say('2',{key:'count'})];
 assert.equal(utterances.length,1);utterances[0].onend();await Promise.resolve();assert.equal(utterances[1].text,'2');voice.cancel();await Promise.all(promises);assert.equal(voice.current,null);assert.equal(voice.queue.length,0);
});
...
test('encouragement yields to counts and does not queue behind tracking guidance',async()=>{
 const utterances=[],voice=queuedVoice(utterances);
 try{
  ...
  utterances.at(-1).onend();await Promise.resolve();assert.equal(voice.current.text,'Halfway.');voice.cancel();await Promise.all([first,halfway]);
