...
if(view){
 ...
 let viewer:CreatureViewer|null=null,lastCaption='',lastCueAt=0,holdUntil=0,epoch=0,closed=false,shown=true,voiceActive=false,walking=false,skinSource:ReturnType<typeof createInstalledCreatureSkinSource>|null=null,skinEpoch=0;
 const tour=new IdleCycle((Object.keys(GESTURES) as Gesture[]).filter(g=>g!=='walk'));
 ...
 async function updateRecipe(raw?:string){const run=++epoch;card.dataset.ready='false';status.textContent='Loading coach…';try{if(!viewer){viewer=new CreatureViewer(stage,base,false);settings();}const recipe=raw?importCreature(raw):loadRecipe(localStorage);const loaded=await viewer.setRecipe(recipe);if(!loaded||run!==epoch||closed)return;card.dataset.ready='true';status.textContent='MYR5';const giant=document.body.dataset.screen==='rest';viewer.setStage(viewer.stage==='overlay'?'overlay':giant?'encounter':'pod');viewer.play(giant?'laugh':'greet');holdUntil=performance.now()+(giant?3700:2800);}catch(error){if(run===epoch&&!closed)status.textContent=(error as Error).message;}}
 async function refreshInstalledSkins(account=window.myr5AuthenticatedAccount){const run=++skinEpoch,owner=typeof account==='string'?account:account?.user?.id;skinSource?.dispose();skinSource=null;viewer?.clearSkinState();viewer?.setSkinResolver(undefined);if(!owner){void updateRecipe();return;}const source=createInstalledCreatureSkinSource({account});const rows=await source.list();const active=window.myr5AuthenticatedAccount,activeOwner=typeof active==='string'?active:active?.user?.id;if(closed||run!==skinEpoch||activeOwner!==owner){source.dispose();return;}if(rows.length){skinSource=source;viewer?.setSkinResolver(source.resolve);}else source.dispose();void updateRecipe();}
 function play(id:Gesture,duration=GESTURES[id]?.duration*1000+300){if(walking&&id!=='walk')return;if(!shown||viewer?.cinematicKind||!Object.hasOwn(GESTURES,id))return;viewer?.play(id);holdUntil=performance.now()+duration;}
 ...
 const timer=setInterval(()=>{const now=performance.now(),busy=closed||!viewer?.motion||card.dataset.ready!=='true'||!shown||document.hidden||!!document.querySelector('dialog[open]')||!!viewer?.cinematicKind||now<holdUntil||voiceActive||document.body.dataset.tracking==='true'||reduced.matches;viewer?.setHomeMotion(!busy);const next=tour.next(now,!busy);if(closed||!viewer?.motion||card.dataset.ready!=='true'||!shown||document.hidden)return;if(next)viewer.play(next);else if(now>holdUntil&&!viewer.cinematicKind&&(voiceActive||document.body.dataset.tracking==='true')){const cue=walking?'walk':voiceActive?'speaking':'listening';if(viewer.motion.current!==cue)viewer.play(cue);}const label=GESTURES[viewer.motion.current].label;if(status.textContent!==label)status.textContent=label;},200);
 const storage=(event:StorageEvent)=>{if(event.key===MOTION_KEY)settings();if(event.key===RECIPE_KEY)void updateRecipe(event.newValue||undefined);if(event.key?.startsWith('myr5-battle-pass-ledger-v1/account/')||event.key==='myr5-battle-pass-ledger-v1')void refreshInstalledSkins();};window.addEventListener('storage',storage);reduced.addEventListener('change',settings);
 const accountReady=(event:Event)=>void refreshInstalledSkins((event as CustomEvent).detail);const accountCleared=()=>void refreshInstalledSkins(null),battlePass=()=>void refreshInstalledSkins();window.addEventListener('myr5:account-ready',accountReady);window.addEventListener('myr5:account-cleared',accountCleared);window.addEventListener('myr5:battle-pass',battlePass);
 ...
 window.addEventListener('pagehide',()=>{closed=true;epoch++;skinEpoch++;clearInterval(timer);observer.disconnect();skinSource?.dispose();viewer?.dispose();window.removeEventListener('myr5:cue',cue);window.removeEventListener('myr5:response',response);window.removeEventListener('storage',storage);window.removeEventListener('myr5:recipe',recipeChanged);window.removeEventListener('myr5:account-ready',accountReady);window.removeEventListener('myr5:account-cleared',accountCleared);window.removeEventListener('myr5:battle-pass',battlePass);reduced.removeEventListener('change',settings);},{once:true});
 // Workout overlay: legs walk while the coach moves across the room; cue gestures wait until it stops.
 (window as any).myr5Creature={play,walk:(on:boolean)=>{if(on===walking)return;walking=on;if(on){holdUntil=0;viewer?.play('walk');}else if(viewer?.motion?.current==='walk')viewer.play('idle');},face:(yaw:number)=>viewer?.face(yaw),stage:(value:'pod'|'encounter'|'overlay')=>viewer?.setStage(value),cinematic:(kind:Cinematic|null,elapsed=0)=>{viewer?.cinematic(kind,elapsed);holdUntil=performance.now()+(kind?10000:500);},stats:()=>({...viewer?.stats(),stage:viewer?.stage,cinematic:viewer?.cinematicKind,handsVersion:2})};void updateRecipe();void refreshInstalledSkins();
