...
function syncShipEditor(){const owned=coachEditorShips(),visible=canShowCoachEditorShipSection();shipTab.hidden=!visible;shipTab.tabIndex=-1;shipTab.setAttribute('aria-hidden',String(!visible));if(!visible&&shipTab.getAttribute('aria-selected')==='true')openMenu(document.getElementById('tab-body') as HTMLButtonElement);const select=document.getElementById('shipChoice') as HTMLSelectElement;if(!select)return;const current=shipSettings();select.replaceChildren(...owned.map(id=>{const option=document.createElement('option');option.value=id;option.textContent=id[0].toUpperCase()+id.slice(1);return option}));const selected=owned.includes(current.ship)?current.ship:owned[0]||'';select.value=selected;($('shipTint') as HTMLInputElement).value=/^#[0-9a-f]{6}$/i.test(current.tint||'')?current.tint:'#ffffff';}
...
async function refreshSkinEditor(account=window.myr5AuthenticatedAccount){
 ...
 const source=createInstalledCreatureSkinSource({account});const rows=await source.list();const active=window.myr5AuthenticatedAccount,activeOwner=typeof active==='string'?active:active?.user?.id;if(run!==skinEpoch||activeOwner!==owner){source.dispose();return;}
 if(!rows.length){source.dispose();render();return;}
 skinSource=source;skinChoices=rows.map(({id,displayName})=>({id,displayName}));viewer?.setSkinResolver(source.resolve);skinTab.hidden=false;skinTab.tabIndex=-1;skinTab.setAttribute('aria-hidden','false');renderSkinChoices();const remembered=skinSettings()[selected];if(typeof remembered==='string'&&skinChoices.some(s=>s.id===remembered)&&recipe.materials?.[selected]?.textureId!==remembered)setMaterial({textureId:remembered});else render('Installed skins ready');
}
